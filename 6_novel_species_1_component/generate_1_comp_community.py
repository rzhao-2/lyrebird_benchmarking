...
if __name__ == '__main__':
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Simulate each fasta
        # for i, fa in enumerate([novel_genome_fasta]):
        logging.info("Simulating reads from %s ..", novel_genome_fasta)
        extern.run(
            f"{args.art} -ss HSXt -i {novel_genome_fasta} -p -l {read_length} -f {coverage} -m 400 -s 10 -o simulated_reads/{i}. &>/dev/null")
