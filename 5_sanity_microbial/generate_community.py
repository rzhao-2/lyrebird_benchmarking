...
if __name__ == '__main__':
    ...
    parent_parser.add_argument('--viral-genome-list', required=True, help='Path to genome list')
    parent_parser.add_argument('--microbial-genome-list', required=True, help='Path to genome list')
    parent_parser.add_argument('--microbial-metadata', required=True, help='Path to microbial metadata file, for genome length')
    ...
    parent_parser.add_argument('--viral-metadata', required=True, help='Path to viral metadata file, for genome length')
    ...
    # coverages for sanity, not mock community, leave coverage files as dummy
    coverages = pd.DataFrame({'coverage': [10]*2})
    logging.info(f"Using {len(coverages)} coverages of 10.")

    # # build viral metadata
    vir_genomes = pd.read_csv(args.viral_genome_list, sep='\t', header=None, names=['accession','fasta'])
    logging.info(f"Read {len(vir_genomes)} genome fasta paths.")

    vir = pl.read_csv(args.viral_metadata, separator='\t', infer_schema_length=100000, ignore_errors=True)
    vir_metadata = pl.concat([
        vir.select('accession', 'genome_size', 'taxonomy'),
    ]).to_pandas()
    logging.info(f"Read {len(vir_metadata)} metadata entries.")

    # Shuffle genomes order so we get randomness
    # vir_metadata = vir_metadata.sample(frac=1).reset_index(drop=True)
    g1 = pd.merge(vir_genomes, vir_metadata, on='accession', how='inner')
    g1 = g1.sample(frac=1).reset_index(drop=True)
    # Make paths absolute so that they work in a tempdir
    g1['fasta'] = g1['fasta'].apply(lambda x: os.path.abspath(x))

    # build microbial metadata
    mic_genomes = pd.read_csv(args.microbial_genome_list, sep='\t', header=None, names=['accession','fasta'])
    logging.info(f"Read {len(mic_genomes)} genome fasta paths.")

    # build and merge microbial metadata
    mic = pl.read_csv(args.microbial_metadata, separator='\t', infer_schema_length=100000, ignore_errors=True)
    ...
    gtdb_metadata = pl.concat([
        mic.select('user_genome', 'genome_size', 'classification'),
        # bac.select('ncbi_genbank_assembly_accession', 'genome_size', 'gtdb_taxonomy'),
        # ar.select('ncbi_genbank_assembly_accession', 'genome_size', 'gtdb_taxonomy'),
    ]).to_pandas()
    # change column name to taxonomy
    # gtdb_metadata = gtdb_metadata.rename(columns={'gtdb_taxonomy': 'taxonomy', 'ncbi_genbank_assembly_accession': 'accession'})
    gtdb_metadata = gtdb_metadata.rename(columns={'user_genome': 'accession', 'classification': 'taxonomy'})
    logging.info(f"Read {len(gtdb_metadata)} metadata entries.")
    
    # Shuffle genomes order so we get randomness
    # gtdb_metadata = gtdb_metadata.sample(frac=1).reset_index(drop=True)
    g2 = pd.merge(mic_genomes, gtdb_metadata, on='accession', how='inner')
    g2 = g2.sample(frac=1).reset_index(drop=True)
    # Make paths absolute so that they work in a tempdir
    g2['fasta'] = g2['fasta'].apply(lambda x: os.path.abspath(x))

    # sample one microbial and one viral genome
    g2 = g2.head(1)
    print(g2)
    g1 = g1.head(1)
    g2 = pd.concat([g1, g2], ignore_index=True)
    print(g2)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(args.output_genomewise_coverage, 'w') as genome_wise_f:
            genome_wise_f.write("accession\tcoverage\tfasta\ttaxonomy\tgenome_size\n")
            with open(output_condensed, 'w') as f:
                f.write("sample\tcoverage\ttaxonomy\n")
                tax_to_coverage = {}
                for i, (fasta, genome_size, taxonomy, coverage) in enumerate(zip(g2['fasta'], g2['genome_size'],g2['taxonomy'], coverages['coverage'])):
                    ...
                    genome_wise_f.write(f"{g2['accession'][i]}\t{coverage}\t{fasta}\t{taxonomy}\t{genome_size}\n")
