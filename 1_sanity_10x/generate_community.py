...
if __name__ == '__main__':
    ...
    coverages = [10, 10, 0, 0, 0, 0, 0, 0, 0]
    logging.info(f"Read {len(coverages)} coverages.")
    ...
    # logging.info(f"After removing plasmids etc, {len(coverages)} coverages remain.")
    # logging.info(f"After removing plasmids etc, {len(coverages)} coverages > 0 remain.")

    genomes = pd.read_csv(args.genome_list, sep='\t', header=None, names=['accession','fasta'])
    ...
    logging.info(f"Read {len(metadata)} metadata entries.")

    # # Shuffle genomes order so we get randomness
    # metadata = metadata.sample(frac=1)
    # genomes = genomes.sample(frac=1)

    g2 = pd.merge(genomes, metadata, on='accession', how='inner')
    g2 = g2.sample(frac=1).reset_index(drop=True)
    g2.insert(4, 'coverage', coverages)
    # print(g2)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(args.output_genomewise_coverage, 'w') as genome_wise_f:
            ...
            with open(output_condensed, 'w') as f:
                ...
                for i, (fasta, genome_size, taxonomy, coverage) in enumerate(zip(g2['fasta'], g2['genome_size'],g2['taxonomy'], g2['coverage'])):
                    ...
                    genome_wise_f.write(f"{g2['accession'][i]}\t{coverage}\t{fasta}\t{taxonomy}\n")
